...
* LD reference: We provide LD data from 3 ancestry (EUR, EAS and AFR) in imputed SNPs and HapMap3 SNPs: [UKB Imputed](https://drive.google.com/drive/folders/1qVw5S3nH0Q6uugp2Dr8izpeEWgS0FRSc?usp=sharing), [UKB HapMap3](https://drive.google.com/drive/folders/16YNhF_uHkOAuHgQjxOZ9JsprPPtDdYt3?usp=share_link). We suggest to download imputed LD same ancestry as the GWAS summary data. Note: The LD data we provided was in little endian format which is widely supported by most hardware. 

If you have downloaded our previous LD data, no need to download it again (althrough the format changed).
